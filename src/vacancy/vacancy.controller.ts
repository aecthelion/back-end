...
export class VacancyController {
  async createVacancy(
    req: Request<any, any, ICreateVacancyBody>
  ): Promise<IVacancy> {
  ...
  ): Promise<{ message: string; vacancy: IVacancy | null }> {
    const { id } = req.params;
    const { status } = req.body;

    const updatedVacancy = await vacancyService.updateVacancy(id, status);

    return { message: "Vacancy application updated", vacancy: updatedVacancy };
  }
